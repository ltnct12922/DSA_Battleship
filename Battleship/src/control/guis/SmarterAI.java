...
public class SmarterAI extends BattleshipAI {
    ...
    /**
     * When true the adjacent moves are evaluated for forming a line with existing
     * ship positions.
     * When false the move is selected at random from valid adjacent moves.
     */
    private boolean preferMovesFormingLine;
    /**
     * When true the random selection of moves will find either the first random
     * move with
     * four adjacent not attacked tiles, or the one with the highest number of not
     * attacked tiles.
    ...
    /**
     * Creates the basic setup for the AI by setting up references to the player's
     * grid,
     * and creates a list of all valid moves.
     *
     * @param playerGrid                    A reference to the grid controlled by
     *                                      the player for testing attacks.
     * @param preferMovesFormingLine        True will enable the smartest version of
     *                                      the AI to try and form lines when
     *                                      attacking ships.
     * @param maximiseAdjacentRandomisation True makes the randomised attacks prefer
     *                                      grid positions that have more not
     *                                      attacked points around them.
     */
    ...
    /**
     * Selects an appropriate move depending on whether any ships were currently hit
     * and not yet destroyed.
     * The AI will choose an attack adjacent to known ship hit locations if a ship
     * has been foumd, otherwise
     ...
     *
     * @return A valid move that is adjacent to shipHits preferring one that forms a
     *         line.
     */
    private Position getSmarterAttack() {
        List<Position> suggestedMoves = getAdjacentSmartMoves();
        for (Position possibleOptimalMove : suggestedMoves) {
            if (atLeastTwoHitsInDirection(possibleOptimalMove, Position.LEFT))
                return possibleOptimalMove;
            if (atLeastTwoHitsInDirection(possibleOptimalMove, Position.RIGHT))
                return possibleOptimalMove;
            if (atLeastTwoHitsInDirection(possibleOptimalMove, Position.DOWN))
                return possibleOptimalMove;
            if (atLeastTwoHitsInDirection(possibleOptimalMove, Position.UP))
                return possibleOptimalMove;
        }
    ...
    /**
     * Searches for the valid move with the most adjacent cells that have not been
     * attacked.
     *
    ...
    /**
     * Counts the number of adjacent cells that have not been marked around the
     * specified position.
     *
     * @param position The position to count adjacent cells.
     * @return The number of adjacent cells that have not been marked around the
     *         position.
     */
    ...
    /**
     * Tests if there are two adjacent ship hits in a direction from a test start
     * point.
     *
     ...
     *
     * @return A list of all valid moves that are adjacent cells to the current ship
     *         hits.
     */
