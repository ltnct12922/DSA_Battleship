...
public class SelectionGrid extends Rectangle {
    ...
    /**
     * Look at variable name -_-.
     */
    public static final int LIMIT_MARKERS_HISTORY = 3;
    /**
     ...
     * Look at variable name -_-.
     */
    private List<Position> markersHistory = new ArrayList<Position>();
    /**
    ...
    public boolean markPosition(Position posToMark) {
        ...
        // mark history
        markersHistory.add(0, posToMark);
        if (markersHistory.size() > LIMIT_MARKERS_HISTORY) {
            markersHistory = markersHistory.subList(0, LIMIT_MARKERS_HISTORY);
        }
    ...
    public boolean unMarkLastPosition() {
        if (markersHistory.size() <= 0) {
            return false;
        }
        Position lastMark = markersHistory.get(0);
        markersHistory.remove(0);
        markers[lastMark.x][lastMark.y].unMark();
        return true;
    }
