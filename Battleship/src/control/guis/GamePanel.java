...
public class GamePanel extends JPanel implements MouseListener, MouseMotionListener {
    ...
    public enum GameState {
        PlacingShips, FiringShots, GameOver
    }

    /**
     * The game gives undo maximum per game
     */
    private int undoLimit;
    /**
     * The remaining time to undo
     */
    private int undoRemaining;
    /**
     * Reference to the status panel to pass text messages to show what is
     * happening.
     */
    ...
    /**
     * Reference to the temporary ship that is being placed during the PlacingShips
     * state.
     */
    ...
    /**
     * The game state to represent whether the player can place ships, attack the
     * computer,
    ...
    /**
     * Initialises everything necessary to begin playing the game. The grids for
     * each player are initialised and
     * then used to determine how much space is required. The listeners are
     * attached, AI configured, and
    ...
    /**
     * Draws the grids for both players, any ship being placed, and the status
     * panel.
     *
    ...
    /**
     * Return to the latest move that the player made
     */
    public void undo() {
        if (undoRemaining <= 0) {
            statusPanel.setTopLine("Undo remaining " + undoRemaining);
            return;
        }
        computer.unMarkLastPosition();
        player.unMarkLastPosition();
        undoRemaining--;
        statusPanel.setTopLine("Undo remaining " + undoRemaining);
    }

    /**
     * Resets all the class's properties back to their defaults ready for a new game
     * to begin.
     */
    public void restart() {
        undoRemaining = undoLimit;
     ...
     * PlacingShip state. Then if the place it has been placed is valid the ship
     * will
    ...
    /**
     * Finalises the insertion of the ship being placed by storing it in the
     * player's grid.
    ...
    /**
     * Constrains the ship to fit inside the grid. Updates the drawn position of the
     * ship,
     * and changes the colour of the ship based on whether it is a valid or invalid
     * placement.
     *
     * @param targetPos The grid coordinate where the ship being placed should
     *                  change to.
     */
    ...
    /**
     * Triggered when the mouse button is released. If in the PlacingShips state and
     * the
     * cursor is inside the player's grid it will try to place the ship.
     * Otherwise if in the FiringShots state and the cursor is in the computer's
     * grid,
    ...
    /**
     * Triggered when the mouse moves inside the panel. Does nothing if not in the
     * PlacingShips state.
     * Will try and move the ship that is currently being placed based on the mouse
     * coordinates.
     *
     * @param e Details about where the mouse event occurred.
     */
